...
contract DeflatePortal {
    ...
    function executeStrategy(
        ...
        _validatePortalCalldata(txData);

        // Execute the swap through the router
        (bool success, bytes memory returnData) = routerAddress.call{value: msg.value}(txData);
        if (!success) revert();
    ...
    function _validatePortalCalldata(
        ...
        if (bytes4(txData[0:4]) == routerFunctionSelector) {
            (orderPayload, partner) = abi.decode(txData[4:], (IPortalsRouter.OrderPayload, address));
        } else {
            (orderPayload, partner) = abi.decode(txData, (IPortalsRouter.OrderPayload, address));
        }
